...
| 12 | `crate::user_level::init()` | User-process, minimal ELF loader, component framework, and FxFS scaffold init log |
...
## 6. Bootstrap Component ELF Loading
...
FxFS is still in memory, but paths now resolve through explicit directory entries backed by object ids. File and directory objects carry mode, uid/gid, size, timestamps, and link count. The shell smoke path also exercises append, truncate, seek/read, attribute lookup, and journal replay metadata.
...
- The Fuchsia-inspired userspace layer now parses boot component ELF images, creates scheduler launcher threads that enter EL0 for bootstrap component payloads, and has a fixed-message `/svc` IPC layer, but it is still not a full component-manager/FIDL/package-resolver/FxFS port.
