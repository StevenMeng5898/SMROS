...
The current verified slice is the standalone proof file at `verification/syscall/src/lib.rs`, which models the overflow-safe address-range helpers and multi-mapping availability predicates used by `src/syscall/syscall.rs`.
