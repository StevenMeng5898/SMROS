...
- [ ] **Step 2: Preserve PID and TID allocation across launch reset**

Modify `tests/host/src/lib.rs`,
`src/syscall/linux_process_logic_shared.rs`, and
`src/syscall/linux_task_logic_shared.rs`.

First change the process and task reset tests to require launch cleanup to
clear every live record without changing the boot-wide allocator high-water
mark or clearing permanent exhaustion. The first child after a reset must use
the next never-issued identity, and an allocator exhausted before reset must
remain exhausted afterward.

Run the focused tests and verify RED:

```bash
./scripts/run-host-unit-tests.sh --lib \
  launch_reset_preserves_boot_tid_high_water -- --nocapture
./scripts/run-host-unit-tests.sh --lib \
  launch_reset_preserves_boot_pid_high_water -- --nocapture
./scripts/run-host-unit-tests.sh --lib \
  allocator_exhaustion_remains_permanent_across_launch_reset -- --nocapture
```

Expected: the high-water tests receive identity 2 instead of the next
identity, and the exhaustion test allocates after reset.

Then make both `reset` methods clear only launch-owned records and wait/signal
state. Preserve `next_pid`, `next_tid`, and both `exhausted` fields so that
allocation remains monotonic for the boot:
...
Rerun all focused tests and the complete host library, then commit:

```bash
./scripts/run-host-unit-tests.sh --lib
git add tests/host/src/lib.rs \
  src/syscall/linux_process_logic_shared.rs \
  src/syscall/linux_task_logic_shared.rs
git commit -m "fix: preserve boot-wide Linux identities"
```

- [ ] **Step 3: Permit execute-only AArch64 mappings at EL0**

Modify `tests/host/src/lib.rs` and
`src/kernel_lowlevel/aarch64_vm_logic_shared.rs`.

Add a descriptor test for `(readable=false, writable=false,
executable=true)` that requires `AARCH64_DESC_AP_USER` and
`AARCH64_DESC_AP_READ_ONLY`, clears `AARCH64_DESC_UXN`, and retains
`AARCH64_DESC_PXN`.
...
```bash
./scripts/run-host-unit-tests.sh --lib \
  execute_only_user_page_is_el0_accessible -- --nocapture
```

Expected: the descriptor is executable but lacks
`AARCH64_DESC_AP_USER`.

Grant user access whenever any user permission is requested while retaining
read-only AP bits for a non-writable page:
...
Rerun the focused descriptor test and complete host library, then commit:

```bash
./scripts/run-host-unit-tests.sh --lib
git add tests/host/src/lib.rs \
  src/kernel_lowlevel/aarch64_vm_logic_shared.rs
git commit -m "fix: permit EL0 execute-only mappings"
```

- [ ] **Step 4: Repeat independent full-range review**

Review `master..HEAD` again against the approved design. Resolve every
Critical and Important finding and rerun the affected focused tests before
continuing.

- [ ] **Step 5: Run the complete pre-merge gate**
...
- [ ] **Step 6: Fast-forward local master and preserve the feature worktree**
