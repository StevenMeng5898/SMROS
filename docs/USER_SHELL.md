...
error, or infrastructure error is reported explicitly. Direct `testsc`, Rust,
and model tests never count as POSIX passes; see `docs/POSIX_CONFORMANCE.md` for
the denominators and current execution limitations.

The runner prints live selection coverage as ordinary serial text alongside
...
and at suite completion. `apis-complete` and `groups-complete` require terminal
results for every selected test in that unit. `apis-pass` and `groups-pass`
also require every selected result to pass. The `scope=selected` marker means
these ratios describe only the current filter. This selection coverage
