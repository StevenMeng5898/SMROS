...
fn linux_signal_bit(signum: usize) -> u64 {
    linux_task::linux_signal_bit(signum)
}
...
fn linux_signal_record_from_user(
    signum: usize,
    info: usize,
...
fn take_selected_linux_signal(
    ...
    Ok(
        selected.map(|(record, reservation)| LinuxDeliverableSignal {
            record,
            source,
...
fn take_unblocked_linux_signal() -> Option<LinuxDeliverableSignal> {
    if let Ok(Some((record, reservation))) = linux_task::take_current_unblocked_signal() {
...
#[derive(Clone, Copy)]
enum LinuxSignalFrameRestore {
    Return,
    Restart(LinuxRestartBlock),
}

fn apply_linux_restart_block(saved_regs: usize, restart: LinuxRestartBlock) {
    let regs = unsafe { &mut *(saved_regs as *mut [u64; 32]) };
...
fn restore_linux_signal_frame(saved_regs: usize) -> Option<LinuxSignalFrameRestore> {
    ...
    *regs = frame.regs;
    ...
    Some(
        frame
            .restart
            .map(LinuxSignalFrameRestore::Restart)
            .unwrap_or(LinuxSignalFrameRestore::Return),
    )
}
...
pub extern "C" fn complete_linux_signal_syscall_return(saved_regs: usize) {
    ...
    let sigreturn_requested =
        linux_task::with_current_signal_state(|signal_state| signal_state.sigreturn_requested)
            .unwrap_or(false);
    let restored = if sigreturn_requested {
        let Some(restored) = restore_linux_signal_frame(saved_regs) else {
            return;
        };
        Some(restored)
    } else {
    ...
    if let Some(LinuxSignalFrameRestore::Restart(restart)) = restored {
        let restart_still_pending = linux_task::with_current_signal_state(|signal_state| {
            signal_state.restart_block == Some(restart)
        })
        .unwrap_or(false);
        if restart_still_pending {
...
fn linux_write_sleep_remaining(address: usize, wait: LinuxSleepWait) -> SysResult {
    let relative = wait.relative.ok_or(SysError::EINVAL)?;
    if address == 0 {
        return Ok(0);
    }
    let size = core::mem::size_of::<LinuxTimespec>();
    ...
    let (tv_sec, tv_nsec) = linux_task::linux_sleep_remaining_timespec(
        relative.started_at,
        relative.requested_nanoseconds,
        now,
...
fn linux_sleep_until(wait: LinuxSleepWait, rem: usize) -> SysResult {
    ...
    if pending {
        let remaining = if wait.relative.is_some() {
...
fn finish_linux_signal_wait(
    ...
    if let Err(error) = copy_linux_signal_wait_info(record, output_address) {
...
pub fn sys_rt_sigprocmask(how: isize, set: usize, oldset: usize, sigsetsize: usize) -> SysResult {
    ...
    if set != 0 {
        ...
        linux_task::with_current_signal_state(|signal_state| match how {
            LINUX_SIG_BLOCK => signal_state.mask |= requested,
            LINUX_SIG_UNBLOCK => signal_state.mask &= !requested,
            LINUX_SIG_SETMASK => signal_state.mask = requested,
            _ => unreachable!(),
        })?;
    }
    Ok(0)
...
pub fn sys_rt_sigreturn() -> SysResult {
    let requested =
        linux_task::with_current_signal_state(|signal_state| signal_state.request_sigreturn())?;
    if !requested {
        return Err(SysError::EINVAL);
    }
    Ok(0)
...
pub fn sys_rt_sigtimedwait(
    ...
    match outcome {
        Some(wait) if wait.outcome == LinuxSignalWaitOutcome::Signal => {
            let (Some(source), Some(reservation)) = (wait.signal_source, wait.signal_reservation)
            else {
                return Err(SysError::EAGAIN);
            };
...
pub fn sys_sigaltstack(ss: usize, old_ss: usize) -> SysResult {
    ...
    if ss != 0 {
        ...
        linux_task::with_current_signal_state(|signal_state| -> Result<(), SysError> {
            if signal_state.alt_stack.flags as u64 & LINUX_SS_ONSTACK != 0 {
                return Err(SysError::EPERM);
            }
            signal_state.alt_stack = if requested.flags as u64 & LINUX_SS_DISABLE != 0 {
